...
// Horario: Lun–Sáb 8:00AM–8:50PM | Dom 8:00AM–2:00PM
function isDentroDeHorario() {
    const ahora = new Date();
    const dia = ahora.getDay(); // 0=Dom, 1=Lun … 6=Sáb
    const totalMin = ahora.getHours() * 60 + ahora.getMinutes();

    if (dia === 0) return totalMin >= 8 * 60 && totalMin < 14 * 60;  // Dom: 8AM–2PM
    if (dia >= 1 && dia <= 6) return totalMin >= 8 * 60 && totalMin < 20 * 60 + 50; // Lun–Sáb: 8AM–8:50PM
    return false;
}

// Texto del tooltip: cuándo reabre el negocio
...
function Contacto() {
    ...
    const handleSubmit = (e) => {
        ...
        // Siempre enviar lo que el usuario escribió en los campos
        const textoWsp =
    ...
    return (
        <ScrollReveal>
        <section className="px-6 md:px-1 py-10" id="contacto">
            <div className="flex gap-5">
                <p id={styleFont.passionOne} className="text-3xl md:text-5xl text-[#B3B6C0]">
                    CONTACTANOS</p>
                <img src={messageIcon} alt="message" className="size-14 md:size-20 animate-bounce" />
            </div>

            {/* RESPONSIVO - columna en mobile, fila en desktop ocupando todo el ancho */}
            <div className="flex flex-col md:flex-row items-stretch gap-10 mt-1 sm:mt-1 w-full">
                ...
                <div className="flex-1 flex justify-center">
                    <div className="bg-white drop-shadow-2xl p-10 w-full relative mt-10">

                        <div className="flex justify-center">
                            <img src={tachuela} alt="tachuela" className="w-20 absolute -top-8" />
                        </div>

                        <form className="text-center flex flex-col gap-6 w-full" onSubmit={handleSubmit} noValidate>
                            <p id={styleFont.jockey} className="text-3xl text-[#F66055] mb-2">
                                Formulario
                            </p>

                            {/* NOMBRE */}
                            <div className="flex flex-col text-left">
                                <label className="font-medium text-gray-700 mb-1">Asunto</label>
                                <input
                                    type="text"
                                    value={nombre}
                                    onChange={(e) => {
                                    ...
                                    className={`rounded-lg px-3 py-2 focus:outline-none focus:ring-2 transition border ${errors.nombre
                            ...
                            <div className="flex flex-col text-left">
                                ...
                                <textarea
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (e.target.value.trim() !== "") setErrors(prev => ({ ...prev, mensaje: false }));
                                    }}
                                    placeholder="¿Cuántas copias necesitas? ¿En qué tamaño?"
                                    className={`rounded-lg px-3 py-2 resize-none h-28 focus:outline-none focus:ring-2 transition border ${errors.mensaje
                                            ? "border-red-500 border-[3px] ring-red-300"
                                            : "border-[#f0686862] focus:ring-[#f06868]"
                                        }`}
                                />
                            ...
                            {/* BOTÓN con tooltip cuando está cerrado */}
                            <div
                            ...
                            >
                                ...
                                {showTooltip && !disponible && (
                                    <div className="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 z-50 w-72 pointer-events-none">
                                        <div className="bg-gray-900 text-white text-sm rounded-xl px-4 py-3 shadow-2xl text-center leading-snug">
                                            🔒 {mensajeReapertura}
                                        </div>
                                        <div className="flex justify-center">
                                            <div className="w-3 h-3 bg-gray-900 rotate-45 -mt-1.5"></div>
                                        </div>
                                    </div>
                                )}
                                ...
                                <button
                                    ...
                                    className={`rounded-full py-3 px-8 font-medium transition duration-200 flex items-center justify-center gap-2 ${
                                        disponible
                                ...
                                >
                                    ...
                                    <img src={whatsapp} alt="whatsapp" className={`w-5 h-5 ${!disponible && "opacity-40"}`} />
                                </button>
                            </div>
